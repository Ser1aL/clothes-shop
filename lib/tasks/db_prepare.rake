namespace :db_prepare do
  ...
  task :categorize => :environment do
    no_brand_sql = <<-SQL
      INSERT INTO countings (category_name, category_id, sub_category_name, sub_category_id, value )
      SELECT IF(categories.display_name IS NOT NULL, categories.display_name, categories.name) as category_name,
        ...
        item_models.sub_category_id, count(*) FROM
        item_models, categories, sub_categories
    ...
    brand_sql = <<-SQL
      INSERT INTO countings (category_name, category_id, sub_category_name, sub_category_id, value, brand_id )
      SELECT IF(categories.display_name IS NOT NULL, categories.display_name, categories.name) as category_name,
        item_models.category_id,
        IF(sub_categories.display_name IS NOT NULL, sub_categories.display_name, sub_categories.name) as sub_category_name,
        item_models.sub_category_id, count(item_models.id), item_models.brand_id
