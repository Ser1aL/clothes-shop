...
namespace :data_feed do
  ...
  task :update_styles => :environment do
    ...
    4000.times do |cycle|
      ...
      output_file.puts "[#{Time.now.to_s(:db)} - Cycle start] Updating #{cycle*100}-#{(cycle+1)*100-1} styles"
      Style.where(:hidden => false).order("original_price ASC").limit(100).offset(100*cycle-1).each_with_index do |style, index|
